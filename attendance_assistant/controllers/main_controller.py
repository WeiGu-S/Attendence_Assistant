...
class MainController:
    ...
    def process_uploaded_image(self, image_path: str) -> bool:
        ...
        try:
            ...
            # 5. 构建数据模型
            self.current_data = self._build_monthly_attendance(attendance_data)
    ...
    def _build_monthly_attendance(self, attendance_data: Dict) -> MonthlyAttendance:
        ...
        try:
            ...
            # 创建每日考勤记录
            daily_records = []
            daily_data_map = {record['day']: record for record in attendance_data.get('daily_records', [])}
            ...
            for day in range(1, days_in_month + 1):
                ...
                clock_in = ClockRecord(
                    time=times.get('clock_in', ''),
                    status=status.get('clock_in_status', '未打卡')
                )
                
                clock_out = ClockRecord(
                    time=times.get('clock_out', ''),
                    status=status.get('clock_out_status', '未打卡')
                )
            ...
            self.logger.info(f"构建月度考勤数据完成: {year_month}")
